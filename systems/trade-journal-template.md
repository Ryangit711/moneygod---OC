# TRADE JOURNAL — Session Record

## "The map is not the territory. The journal is where you find out which part wasn't."

---

## PRE-SESSION (fill before first trade — from live-data-workflow verdict)

**Date:** YYYY-MM-DD
**Session:** [London / NY AM / NY PM / Asia]

| Field | Value |
|-------|-------|
| Trade Mode | FULL / NORMAL / REDUCED / FLAT |
| Instrument | MES / MNQ / BOTH / NONE |
| Bias | BULLISH / BEARISH / NEUTRAL |
| Net Liquidity | $_____B (INJECT / NEUTRAL / DRAIN) |
| Active Edges | _____ , _____ , _____ |
| Session Plan | Line 1: ________________________ |
| | Line 2: ________________________ |

---

## POST-SESSION — TRADE LOG

| # | Time | Instrument | Concept | Setup | Entry | Stop | Exit | R:R | $Result | Screenshot? |
|---|------|-----------|---------|-------|-------|------|------|-----|---------|-------------|
| 1 | | | | | | | | :1 | $ | Y/N |
| 2 | | | | | | | | :1 | $ | Y/N |
| 3 | | | | | | | | :1 | $ | Y/N |
| 4 | | | | | | | | :1 | $ | Y/N |
| 5 | | | | | | | | :1 | $ | Y/N |

**Session P&L:** $_____  **Total R:** ____R

---

## MODEL CHECK

| Question | Answer |
|----------|--------|
| Plumbing verdict match reality? | Y / N / PARTIAL |
| If N: what did the model miss? | |
| Bias correct? | Y / N |
| If N: what bias would have won? | |
| Active edges triggered? | Y / N — which one(s)? |
| Did price confirm the plumbing read? | Y / N / LAG (price caught up later) |

---

## CROSS-ASSET VERIFICATION (end-of-session)

| Market | What It Did | Confirm/Diverged |
|--------|------------|-----------------|
| Equities (MES actual) | | — |
| DXY | | |
| VIX | | |
| Bonds (ZN / yield) | | |
| Commodities (CL / GC) | | |

**End-of-session verdict:** All confirmed / Mixed / Diverged

---

## CYCLES CHECK

| Cycle | Active? | Impact |
|-------|---------|--------|
| Option expiry (opex) | Y/N — next: ____ | |
| Quarter-end rebalance | Y/N — next: ____ | |
| FOMC blackout | Y/N — next: ____ | |
| NFP / CPI week | Y/N — which day: ____ | |
| Tax season (US) | Y/N — dates: ____ | |
| Holiday / reduced volume | Y/N — which: ____ | |
| Earnings season (major names) | Y/N — which: ____ | |

---

## PSYCHE

| Metric | Pre-Session (1-10) | Post-Session (1-10) |
|--------|-------------------|--------------------|
| Confidence | /10 | /10 |
| Discipline | /10 | /10 |
| Anxiety | /10 | /10 |

**Impulse trades?** Y / N
  If Y: what triggered the impulse? _____

**Forced yourself to stay flat when bias said no?** Y / N

**One word for your emotional state right now:** _____

---

## REAL ECONOMY & ON-CHAIN (1 line each — only if notable today)

**Real economy signal** (supply chain, CPI, jobs, energy, shipping, crop, geopolitical): _____

**On-chain / stablecoin signal** (USDC supply, ETF flow, BTC perpetual funding, DeFi TVL trend): _____

---

## ONE LESSON FROM THIS SESSION (mandatory — write one sentence)

> ________________________________________________________________

---

## WEEKLY REVIEW (fill Friday close)

### Performance by Day

| Metric | Mon | Tue | Wed | Thu | Fri | Total |
|--------|-----|-----|-----|-----|-----|-------|
| Sessions traded | /1 | /1 | /1 | /1 | /1 | /5 |
| Total P&L | $ | $ | $ | $ | $ | $ |
| Win rate | % | % | % | % | % | % |
| Avg R:R | :1 | :1 | :1 | :1 | :1 | :1 |
| Impulse trades | | | | | | |
| Model failures | | | | | | |
| Discipline score (1-10) | | | | | | |

### Discipline Metrics (Critical — Flat Is a Win)

| Metric | Count | Notes |
|--------|-------|-------|
| Days you WANTED to trade but didn't (signal wasn't there) | | |
| Days you traded MORE than your session plan allowed | | |
| Times you moved a stop wider after entry | | |
| Times you added to a loser | | |
| **Discipline grade for the week:** A / B / C / D / F | | |

### Edge × Concept Cross-Tracking

For each trade this week, note the plumbing context when it happened:

| Concept (from syllabus) | Trades | Wins | Losses | Plumbing Context That Was Active | Notes |
|------------------------|--------|------|--------|----------------------------------|-------|
| Example: Week 1 Sweep | 3 | 2 | 1 | RRP draining, TGA flat, Net Liq -$20B | Sweeps won when Net Liq was negative |

**Which setup won most this week?** _____

**Which plumbing signal was wrong most often?** _____

**Is there a plumbing × concept combination that stands out?** (e.g., "sweeps win 80% when RRP is draining but 30% when TGA is building"):

> _____

### Psyche & Lessons

**Psyche trend (did discipline degrade across the week?):** _____

**Aha insights captured this week** (things that didn't fit a journal entry but you noticed):

> _____

**Go-to next week or repeat this concept?** _____

**One sentence for future-you reading this entry:** _____

---

## REFERENCE MAP

| Section | Closes Gap |
|---------|-----------|
| Model Check | Model-failure protocol — explicit Y/N on map vs territory |
| Cross-Asset Verification | Cross-asset confirmation — read systematically, not ad-hoc |
| Cycles Check | Multi-week cycles — visible at a glance before every session |
| Psyche | Trader's psyche — track the internal trend, not just the external |
| Real Economy & On-Chain | Physical economy + parallel plumbing — one line keeps both on radar |
| Weekly Review | Reverse causality — patterns emerge across sessions, not in one |

## Synaptic Connections

| Neuron | Synapse | Fire When |
|--------|---------|-----------|
| `systems/weekly-flow-checklist.md` | Journal's weekly review aggregates daily data from the flow checklist | Completing weekly review |
| `core/plumbing-esoterica.md` | Model Check section validates the plumbing verdict against actual market action | Running model check |
| `mental-models/pipe-theory.md` | Psyche and discipline metrics track whether trader stayed in or fought the flow | Assessing trade discipline |
| `systems/position-sizing-by-flow.md` | Trade log R:R and $Result verify whether position sizing rules were followed | Reviewing trade results |
| `systems/sunday-prep-ritual.md` | Pre-session bias and key levels entered in journal stem from Sunday weekly prep | Filling pre-session section |
